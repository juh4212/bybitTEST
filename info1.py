from pybit import HTTP
import pandas as pd
import time

# Bybit API 세션 생성 (인증이 필요한 경우 API_KEY와 API_SECRET을 추가하세요)
# 예: HTTP("https://api.bybit.com", api_key="YOUR_API_KEY", api_secret="YOUR_API_SECRET")
session = HTTP("https://api.bybit.com")

# 현재 시간의 Unix timestamp (초 단위)
current_time = int(time.time())

# 7일 전의 Unix timestamp 계산 (168시간 * 3600초)
start_time = current_time - 168 * 3600

# 7일치 1시간 간격의 데이터 가져오기
response = session.query_kline(
    symbol="BTCUSDT",
    interval=60,  # 60분 간격
    limit=168,
    from_time=start_time
)

# 응답 확인
if 'result' not in response:
    raise ValueError("API 응답에 'result' 키가 없습니다. 응답 내용을 확인하세요.")

data = response['result']

# 데이터가 비어있을 경우 처리
if not data:
    raise ValueError("데이터를 가져오지 못했습니다. API 응답을 확인하세요.")

# 데이터프레임으로 변환
df = pd.DataFrame(data)

# 데이터프레임의 열 확인
print("데이터프레임의 열 확인:", df.columns)

# 필요한 열 선택 및 데이터 처리
columns = ['open', 'high', 'low', 'close', 'volume', 'timestamp']
df_hourly = df[columns].copy()

# 'timestamp'을 datetime 형식으로 변환하고 인덱스로 설정
df_hourly['start_at'] = pd.to_datetime(df_hourly['timestamp'], unit='s')
df_hourly.set_index('start_at', inplace=True)

# 필요한 열만 선택하고 데이터 타입을 float으로 변환
df_hourly = df_hourly[['open', 'high', 'low', 'close', 'volume']].astype(float)

# NaN 값 제거
df_hourly = df_hourly.dropna()

# 결과 출력
print(df_hourly.tail())

# 7일치 데이터가 정상적으로 로드되었는지 확인
print(f"총 데이터 포인트 수: {len(df_hourly)}")
